"""
streamlit_app/pages/5_🤖_AI_Coach.py
--------------------------------------
...
Layout:
  ...
  2. [Generate] button
  3. Deterministic alerts (calculated locally — no LLM cost)
  4. Expander: raw context sent to Claude (pedagogical / portfolio)
  5. LLM response rendered as Markdown (four structured sections)
"""
...
# ---------------------------------------------------------------------------
# Add the streamlit_app parent directory to sys.path so we can import
# from utils/ and the ai_engine/ sibling directory.
...
# Minimal custom CSS — consistent with the rest of the dashboard
st.markdown("""
<style>
    .coach-hero {
...
<div class="coach-hero">
    ...
    <p>Personalised analysis powered by Claude — based on your last 4 weeks of training and health data.</p>
</div>
""", unsafe_allow_html=True)

# ---------------------------------------------------------------------------
# Load data (cached in session state to avoid re-querying on every rerun)
# ---------------------------------------------------------------------------

if "training_summary" not in st.session_state:
    # Load all weeks — build_coaching_context() will take the last 8 internally
...
with col1:
    goal_distance = st.selectbox(
        ...
        index=2,                          # default: Half Marathon
        help="Select the distance you are training for."
    )

with col2:
    # Time inputs split into hours + minutes for clarity
...
with col3:
    # Default race date: 10 weeks from today
    default_race_date = date.today() + timedelta(weeks=10)
    race_date = st.date_input(
        "Race date",
        value=default_race_date,
        min_value=date.today() + timedelta(days=1),
        help="When is your target race? This determines how many weeks are left to train."
    )

# Show the required pace immediately so the user understands what they're targeting
distance_km_map = {"5K": 5.0, "10K": 10.0, "Half Marathon": 21.0975, "Marathon": 42.195}
...
st.info(
    ...
    f"({(race_date - date.today()).days // 7} weeks to go)"
)
...
if generate:
    ...
    # --- Alerts (deterministic — no API cost) ---
    alerts = calculate_alerts(ctx)

    if alerts:
        st.subheader("⚠️ Signals Detected")
        for alert in alerts:
            css_class = "alert-red" if alert.level == "red" else "alert-yellow"
            st.markdown(
                f'<div class="{css_class}">{alert.emoji} {alert.message}</div>',
                unsafe_allow_html=True,
            )
        st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
    else:
        st.markdown(
            '<div class="alert-info">✅ No major training or health alerts detected. '
            'Your load and recovery metrics look balanced.</div>',
            unsafe_allow_html=True,
        )
        st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)

    # --- Expander: raw context sent to Claude (pedagogical) ---
    with st.expander("📋 Data sent to Claude — click to inspect", expanded=False):
        st.markdown(
            "This is the exact context string sent to the AI. "
            "Aggregated metrics are used (not raw rows) to keep the prompt compact and cost-efficient."
        )
        st.code(build_prompt(ctx), language="text")

    # --- Claude API call ---
    st.subheader("🤖 Coach Analysis")

    with st.spinner("Asking Claude for your personalised analysis… (this takes 5–10 seconds)"):
        try:
            # Returns (markdown_response, model_name) tuple
            response_md, model_used = get_coaching_analysis(ctx)
        except RuntimeError as e:
            st.error(str(e))
            st.stop()

    # Render the four-section markdown response
...
# ---------------------------------------------------------------------------
# Empty state — shown before the user clicks Generate
# ---------------------------------------------------------------------------
    ...
    <div style="text-align: center; padding: 3rem 1rem; color: #718096;">
        ...
        <p style="font-size: 1.1rem;">Set your goal above, then click <strong>Generate coaching analysis</strong>.</p>
